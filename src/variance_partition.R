...
# Remove single-level categories (would break the mixed model)
to_remove_vars <- c()
for (cat in VP_CATEGORICAL) {
...
# ─── Collinearity check ──────────────────────────────────────────────────────
# canCorPairs requires a flat formula (no random effects)
...
# TPM > 1 in at least 10% of samples
...
######################################################
# Variance Partition — Per Dataset
######################################################

# Collapsed parent-level batches excluded to avoid double-counting samples
